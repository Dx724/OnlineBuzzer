	...
	var newRoomObject = {
		...
		currentGame: {
			...
			input: function(playerNumber, button, state) {
				if (playerNumber == 1 && button == 1 && state == 0) { //"RESTART" button released
					this.startTime = getTime();
					this.roundPlayers = [];
					this.draw();
				}
				else {
					if (button == 0 && state == 1) { //"BUZZ" button pressed
						this.roundPlayers.push(String(newRoomObject.controllers[playerNumber - 1].data_name) + " " + String((getTime() - this.startTime) / 1000));
						this.draw();
					}
				}
			},
			draw: function() {
				for (var user of newRoomObject.controllers) {
					user.emit("updateDisplay", JSON.stringify(this.roundPlayers));
				}
				console.log(JSON.stringify(this.roundPlayers));
			},
...
socketsModule.on("connection", function(socket) {
	...
	socket.on("setName", function(message) {
		if (message != "" && message.trim() != "") {
			socket.data_name = message;
			socket.emit("nameStatus", "SUCCESS");
		}
		else {
			socket.emit("nameStatus", "FAILED");
		}
	});
	socket.on("roomJoin", function(message) {
		if (message == "new") {
			...
			if (roomNumber == NaN || !rooms.hasOwnProperty(String(roomNumber))) {
				...
				for (player of rooms[String(roomNumber)].controllers) {
					if (player.data_name == socket.data_name) {
						namePresent = true;
					}
				}
				if (namePresent) {
					socket.emit("roomStatus", "FAILED");
					console.log("Request failed (name present in room already) with room number: " + message + ".");
				}
				else {
